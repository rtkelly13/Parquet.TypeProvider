...
type ParquetRow(batch: ParquetColumnBatch, rowIndex: int) =
    ...
    override this.Equals(other: obj) =
        match other with
        | :? ParquetRow as r ->
            if this.Batch.Columns.Length <> r.Batch.Columns.Length then false
            else
                let rec check i =
                    if i >= this.Batch.Columns.Length then true
                    else
                        let v1 = this.GetValue(i)
                        let v2 = r.GetValue(i)
                        if Object.Equals(v1, v2) then check (i + 1)
                        else false
                check 0
        | _ -> false
        ...
        member this.CompareTo(other: obj) =
            ...
            | :? ParquetRow as r ->
                let rec compareCol i =
                    if i >= this.Batch.Columns.Length then 0
                    else
                        let v1 = this.GetValue(i) :?> IComparable
                        let v2 = r.GetValue(i)
                        let c = if obj.ReferenceEquals(v1, null) && obj.ReferenceEquals(v2, null) then 0
                                elif obj.ReferenceEquals(v1, null) then -1
                                elif obj.ReferenceEquals(v2, null) then 1
                ...
                if this.Batch.Columns.Length <> r.Batch.Columns.Length then false
                    ...
                    let rec check i =
                        if i >= this.Batch.Columns.Length then true
                            ...
                            let v1 = this.GetValue(i)
                            let v2 = r.GetValue(i)
                            if comparer.Equals(v1, v2) then check (i + 1)
                            else false
                    check 0
            | _ -> false
