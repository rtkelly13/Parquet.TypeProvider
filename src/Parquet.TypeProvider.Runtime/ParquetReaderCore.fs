...
module ParquetReaderCore =
    ...
    /// Reads a single column chunk from a row group into a boxed typed array.
    let readColumnData (groupReader: ParquetRowGroupReader) (field: DataField) (rowCount: int) : Task<Array> =
        task {
    ...
    /// Reads all row groups from a Stream into a sequence of ParquetRow elements.
    let readRows (stream: Stream) (preferOption: bool) : seq<ParquetRow> =
        seq {
            let readerTask = ParquetReader.CreateAsync(stream, null, true)
            let reader = readerTask.GetAwaiter().GetResult()
            ...
            for i = 0 to reader.RowGroupCount - 1 do
                ...
                let columnArrays =
                    rawFields
                    |> Array.map (fun field ->
                        let readTask = readColumnData groupReader field rowCount
                        let rawArray = readTask.GetAwaiter().GetResult()
    ...
    let readColumnArray<'T> (filePath: string) (columnName: string) : 'T[] =
        use fileStream = File.OpenRead(filePath)
        let readerTask = ParquetReader.CreateAsync(fileStream, null, true)
        let reader = readerTask.GetAwaiter().GetResult()
        let field =
            reader.Schema.GetDataFields()
        ...
        for i = 0 to reader.RowGroupCount - 1 do
            ...
            let rowCount = int groupReader.RowCount
            let readTask = readColumnData groupReader field rowCount
            let colData = readTask.GetAwaiter().GetResult() :?> 'T[]
            Array.Copy(colData, 0, result, offset, rowCount)
